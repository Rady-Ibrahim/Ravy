...
@section('content')
    <div class="mx-auto max-w-2xl">
        <form action="{{ route('admin.users.update', $user) }}" method="post" class="admin-card border-0 shadow-md shadow-slate-200/50 space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-slate-700">{{ __('Name') }}</label>
                <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="email" class="block text-sm font-medium text-slate-700">{{ __('Email') }}</label>
                <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                @error('email')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="password" class="block text-sm font-medium text-slate-700">{{ __('New password') }} <span class="font-normal text-slate-500">({{ __('leave blank to keep') }})</span></label>
                <input type="password" name="password" id="password" autocomplete="new-password" class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
            ...
            <div>
                ...
                <input type="password" name="password_confirmation" id="password_confirmation" autocomplete="new-password" class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
            </div>

            <div>
                <label for="phone" class="block text-sm font-medium text-slate-700">{{ __('Phone') }}</label>
                <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select name="type" id="type" required class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                        <option value="admin" @selected(old('type', $user->type) === 'admin')>{{ __('Admin') }}</option>
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                        ...
                        <option value="banned" @selected(old('status', $user->status) === 'banned')>{{ __('Banned') }}</option>
                    </select>
            ...
            @php
                $oldRoles = old('roles', $user->roles->pluck('name')->all());
            @endphp
            <div id="roles-field" class="{{ old('type', $user->type) === 'customer' ? 'hidden' : '' }}">
                ...
                <div class="mt-2 max-h-48 space-y-2 overflow-y-auto rounded-xl border border-slate-200 p-3">
                    @foreach ($roles as $role)
                        <label class="flex items-center gap-2 text-sm text-slate-700">
                            <input type="checkbox" name="roles[]" value="{{ $role->name }}" @checked(in_array($role->name, $oldRoles, true)) class="rounded border-slate-300 text-amber-600 focus:ring-amber-500">
                            {{ $role->name }}
                        </label>
                    @endforeach
                </div>
