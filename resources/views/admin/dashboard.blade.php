...
@section('content')
    ...
    <div class="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
        <div
            class="admin-card group border border-brand-navy/[0.07] !p-4 sm:!p-5 {{ $dashShadow }} transition hover:shadow-[0_14px_44px_-12px_rgba(0,27,54,0.18),0_6px_16px_-4px_rgba(0,27,54,0.1)]">
        ...
        <div
            ...
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-wide text-brand-navy/45">{{ __('Products') }}
                    </p>
                    <p class="mt-2 text-3xl font-bold tabular-nums text-brand-navy"
                        style="font-family: Outfit, sans-serif;">{{ $kpi['products'] ?? '—' }}</p>
        ...
        <div
            ...
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-wide text-brand-navy/45">{{ __('Orders') }}
                    </p>
                    <p class="mt-2 text-3xl font-bold tabular-nums text-brand-navy"
                        style="font-family: Outfit, sans-serif;">{{ $kpi['orders'] ?? '—' }}</p>
        ...
        <div
            class="admin-card group relative overflow-hidden border border-brand-navy/[0.07] !p-4 sm:!p-5 {{ $dashShadow }} transition hover:shadow-[0_14px_44px_-12px_rgba(0,27,54,0.18),0_6px_16px_-4px_rgba(0,27,54,0.1)]">
            <div class="relative flex items-start justify-between gap-3">
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-wide text-brand-navy/45">{{ __('Revenue') }}
                    </p>
                ...
                <span
                    class="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-navy text-brand-cream shadow-md shadow-brand-navy/25">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                        aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
        <div class="min-w-0 space-y-3 sm:space-y-4">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:items-start sm:gap-4">
                <div class="self-start rounded-2xl border border-brand-navy/10 bg-white p-4 sm:p-5 {{ $dashShadow }}">
                    ...
                    <p class="mt-2 min-h-[2.5rem] text-3xl font-bold leading-tight tabular-nums text-brand-navy sm:text-4xl"
                        style="font-family: Outfit, sans-serif;">{{ $orders['total'] ?? '—' }}</p>
                    <a href="#"
                        class="mt-4 inline-flex w-full items-center justify-center rounded-full bg-brand-navy px-4 py-2.5 text-xs font-semibold text-brand-cream shadow-sm shadow-brand-navy/20 transition hover:bg-brand-navy-mid"
                ...
                <div class="grid grid-cols-1 gap-3">
                    <div
                        class="flex items-center justify-between gap-3 rounded-2xl border border-rose-200/80 bg-gradient-to-r from-rose-50 to-[#fff5f5] px-4 py-3 shadow-sm">
                        ...
                        <span class="text-xl font-bold tabular-nums text-brand-navy"
                            style="font-family: Outfit, sans-serif;">{{ $orders['pending'] ?? '—' }}</span>
                    </div>
                    <div
                        class="flex items-center justify-between gap-3 rounded-2xl border border-emerald-200/80 bg-gradient-to-r from-emerald-50 to-[#f0faf4] px-4 py-3 shadow-sm">
                        <span class="text-sm font-medium text-brand-navy">{{ __('Confirmed') }}</span>
                        <span class="text-xl font-bold tabular-nums text-brand-navy"
                            style="font-family: Outfit, sans-serif;">{{ $orders['confirmed'] ?? '—' }}</span>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div
                            class="rounded-2xl border border-fuchsia-200/70 bg-fuchsia-50/80 px-3 py-3 text-center shadow-sm">
                            <p class="text-[10px] font-semibold uppercase tracking-wide text-brand-navy/50">
                                {{ __('Processed') }}</p>
                            <p class="mt-1 text-lg font-bold text-brand-navy" style="font-family: Outfit, sans-serif;">
                                {{ $orders['processed'] ?? '—' }}</p>
                        </div>
        ...
        <div class="min-w-0">
            <div
                class="admin-card flex min-h-0 min-w-0 flex-col border border-brand-navy/[0.07] !p-4 sm:!p-5 {{ $dashShadow }}">
                    ...
                    @foreach ($categories as $cat)
                        <li>
                            ...
                            <div class="mt-2 h-2 overflow-hidden rounded-full bg-brand-navy/8">
                                <div class="h-full rounded-full bg-gradient-to-l from-brand-navy to-brand-navy-soft transition-all"
                                    style="width: {{ max(8, $cat['pct']) }}%"></div>
                            </div>
                        </li>
                    @endforeach
                </ul>
                <p class="mt-4 border-t border-brand-navy/10 pt-3 text-center text-xs text-brand-navy/40">
                    {{ __('Live rankings will populate from reports.') }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="admin-card min-w-0 border border-brand-navy/[0.07] !p-4 sm:!p-5 {{ $dashShadow }}">
            ...
            <div class="mt-6 grid min-w-0 grid-cols-1 gap-6 sm:grid-cols-[10rem_minmax(0,1fr)] sm:gap-5">
                <div class="relative isolate flex justify-center sm:justify-start">
                    <div class="relative h-36 w-36 shrink-0 sm:h-40 sm:w-40" role="img"
                        aria-label="{{ __('Revenue share chart placeholder') }}">
                        <div class="absolute inset-0 rounded-full shadow-inner shadow-brand-navy/10"
                            style="background: conic-gradient(#001b36 0deg 260deg, #c4a574 260deg 318deg, #dfe8e4 318deg 360deg);">
                        </div>
                        <div
                            class="absolute inset-[14%] z-10 flex items-center justify-center rounded-full bg-white text-center shadow-md ring-1 ring-brand-navy/5">
                            <div>
