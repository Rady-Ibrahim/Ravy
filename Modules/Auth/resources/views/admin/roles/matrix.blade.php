    ...
    <div id="matrix-page-root" class="mx-auto max-w-[100rem] space-y-6">
        @if ($catalogPermissions)
            <div class="flex flex-wrap gap-2" role="tablist" aria-label="{{ __('Matrix page sections') }}">
                <button
                    type="button"
                    data-page-mode="matrix"
                    data-active="{{ $catalogPanelOpen ? 'false' : 'true' }}"
                ...
                <button
                    type="button"
                    data-page-mode="catalog"
                    data-active="{{ $catalogPanelOpen ? 'true' : 'false' }}"
                    class="rounded-xl border px-3 py-2 text-sm font-semibold transition data-[active=true]:border-amber-500 data-[active=true]:bg-amber-50 data-[active=true]:text-amber-950 data-[active=false]:border-slate-200 data-[active=false]:bg-white data-[active=false]:text-slate-600 data-[active=false]:hover:border-slate-300"
                >
        ...
        <div id="matrix-shell" @class(['space-y-6', 'hidden' => $catalogPanelOpen && $catalogPermissions])>
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div class="w-full max-w-md">
                    ...
                    <input
                        ...
                        placeholder="{{ __('Filter by label or permission name…') }}"
                        autocomplete="off"
                    >
                </div>
            </div>
            ...
            <form action="{{ route('admin.roles.matrix.update') }}" method="post" id="permission-matrix-form">
                ...
                <div class="admin-card border-0 p-0 shadow-md shadow-slate-200/50">
                    <div class="max-h-[min(70vh,720px)] overflow-auto rounded-2xl border border-slate-100">
                        <table class="w-full min-w-[720px] border-collapse text-left text-sm">
                            <thead class="sticky top-0 z-30 bg-slate-50/95 text-xs font-semibold uppercase tracking-wide text-slate-500 backdrop-blur">
                                <tr>
                                    <th scope="col" class="sticky left-0 z-40 min-w-[11rem] border-b border-r border-slate-200 bg-slate-50 px-3 py-3 align-bottom text-slate-600 shadow-[4px_0_12px_-4px_rgba(15,23,42,0.12)]">
                                        {{ __('Role') }}
                                    </th>
                                    @foreach ($modules as $module)
                                                ...
                                                >
                                                    <div class="flex min-w-[4.25rem] flex-col items-center gap-1">
                                                        @if ($modLabel !== '' && $grpLabel !== '' && $modLabel !== $grpLabel)
                                                            <span class="text-[10px] font-bold uppercase tracking-wide text-slate-400">{{ __($modLabel) }}</span>
                                                            <span class="text-[10px] font-semibold leading-tight text-slate-500">{{ __($grpLabel) }}</span>
                                                        @else
                                                            <span class="text-[10px] font-bold uppercase tracking-wide text-slate-400">{{ __($modLabel !== '' ? $modLabel : $grpLabel) }}</span>
                                                        @endif
                                                        <span class="text-xs font-semibold text-slate-800">{{ __($perm['label']) }}</span>
                                                        <button
                                                            type="button"
                            ...
                            <tbody class="divide-y divide-slate-100 bg-white">
                                @foreach ($roles as $role)
                                    ...
                                    <tr class="hover:bg-slate-50/60" data-role-row="{{ $role->id }}">
                                        <th scope="row" class="sticky left-0 z-20 border-r border-slate-100 bg-white px-3 py-2.5 text-left font-medium text-slate-900 shadow-[4px_0_12px_-4px_rgba(15,23,42,0.08)]">
                                            <div class="flex flex-col gap-2">
                                                <div class="flex flex-wrap items-center gap-2">
                                                    <span class="truncate">{{ $role->name }}</span>
                                                    @if ($isSuperAdmin)
                                                        <span class="inline-flex items-center rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-800">{{ __('All permissions') }}</span>
                                                    @endif
                                                </div>
                                                @unless ($isSuperAdmin)
                                                    <button
                                        ...
                                        @foreach ($modules as $module)
                                            @foreach ($module['groups'] ?? [] as $group)
                                                @foreach ($group['permissions'] ?? [] as $perm)
                                                    @php
                                                        $checked = $isSuperAdmin ? true : $role->hasPermissionTo($perm['name']);
                                                    @endphp
                                                    <td
                                                        class="matrix-cell border-slate-50 px-1 py-1.5 text-center align-middle"
                                                        data-matrix-module="{{ $module['key'] }}"
                                                        data-matrix-perm="{{ $perm['name'] }}"
                                                    >
                                                        @if ($isSuperAdmin)
                ...
                <div class="mt-6 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800">
                        {{ __('Save changes') }}
                    </button>
                </div>
            </form>
                ...
                <div class="grid gap-6 lg:grid-cols-3 lg:items-start">
                    <div class="admin-card overflow-hidden border-0 p-0 shadow-md shadow-slate-200/50 lg:col-span-2">
                        <div class="border-b border-slate-100 px-4 py-3">
                            <h2 class="text-sm font-semibold text-slate-900" style="font-family: Outfit, sans-serif;">{{ __('Registered permissions') }}</h2>
                            <p class="mt-0.5 text-xs text-slate-500">{{ __('Web guard — used by the admin panel and matrix.') }}</p>
                        </div>
                    ...
                    @can('admin.permissions.create')
                        <div id="permission-catalog-form" class="admin-card border-0 shadow-md shadow-slate-200/50">
                            <h2 class="text-base font-semibold text-slate-900" style="font-family: Outfit, sans-serif;">{{ __('Add permission') }}</h2>
                            <p class="mt-1 text-xs text-slate-500">{{ __('Dot notation, e.g. module.action. Guard is fixed to web.') }}</p>
                            <form action="{{ route('admin.permissions.store') }}" method="post" class="mt-4 space-y-4">
                                @csrf
                                <div>
                                    <label for="catalog-permission-name" class="block text-sm font-medium text-slate-700">{{ __('Permission name') }}</label>
                                    <input
                                        type="text"
                                        name="name"
                                        id="catalog-permission-name"
                                        value="{{ old('name') }}"
                                        required
                                        ...
                                        placeholder="admin.reports.view"
                                        autocomplete="off"
                                    >
    ...
    <script>
        (function () {
            ...
            function refreshColState(perm) {
                ...
                var on = vis.filter(function (i) { return i.checked; }).length;
                if (on === vis.length) {
            ...
            form.querySelectorAll('.matrix-col-toggle').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    ...
                    var allOn = vis.every(function (i) { return i.checked; });
                    var next = !allOn;
