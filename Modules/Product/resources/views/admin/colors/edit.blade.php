...
@section('title', __('Edit Color') . ' - ' . $color->name)
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" id="code" name="code" value="{{ old('code', $color->code) }}" 
                                   class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
                            @error('code')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex items-center gap-3">
                                <input type="color" id="color-picker" value="{{ old('hex', $color->hex ?: '#000000') }}"
                                       class="h-10 w-20 rounded-lg border border-slate-300 cursor-pointer">
                                <input type="text" id="hex" name="hex" value="{{ old('hex', $color->hex) }}" placeholder="#FF0000"
                        ...
                        <div>
                            ...
                            @if ($color->image)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $color->image) }}" alt="{{ $color->name }}" class="h-20 w-20 rounded-lg object-cover">
                        ...
                        <div>
                            ...
                            <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $color->sort_order) }}" min="0"
                                   class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            @error('sort_order')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
