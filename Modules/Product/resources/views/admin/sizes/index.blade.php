...
@section('content')
    <div class="mx-auto max-w-7xl">
        <div class="mb-4">
            <a href="{{ route('admin.sizes.create') }}" class="inline-flex items-center justify-center rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($sizes as $size)
                            <tr class="bg-white hover:bg-slate-50/50">
                                <td class="px-4 py-3">
                                    @if ($size->image)
                                        <img src="{{ asset('storage/' . $size->image) }}" alt="{{ $size->name }}" class="h-10 w-10 rounded-lg object-cover">
                                    @else
                                ...
                                <td class="px-4 py-3 text-end">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('admin.sizes.edit', $size) }}" class="rounded-lg px-2 py-1 text-xs font-semibold text-amber-700 hover:bg-amber-50">{{ __('Edit') }}</a>
                                        <form action="{{ route('admin.sizes.destroy', $size) }}" method="post" class="inline" onsubmit="return confirm(@json(__('Delete this size?')));">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-slate-500">{{ __('No sizes found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
