...
@section('page_title', __('Edit attribute'))
@section('page_subtitle', $attribute->name)

@section('content')
    <div class="mx-auto max-w-3xl">
        <form action="{{ route('admin.attributes.update', $attribute) }}" method="post" class="admin-card space-y-5">
            @csrf
            @method('PUT')
            @include('product::admin.attributes.partials.form', ['attribute' => $attribute])
