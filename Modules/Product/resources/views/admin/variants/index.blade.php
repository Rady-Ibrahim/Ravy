...
@section('content')
    <div class="mx-auto max-w-7xl space-y-4">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.products.index') }}" class="text-sm font-medium text-slate-600 hover:text-slate-900">{{ __('Back to products') }}</a>
            <a href="{{ route('admin.products.variants.create', $product) }}" class="inline-flex items-center justify-center rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800">{{ __('Add variant') }}</a>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($variants as $variant)
                            <tr class="bg-white hover:bg-slate-50/50">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset {{ $variant->is_active ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20' : 'bg-slate-100 text-slate-600 ring-slate-500/10' }}">
                                        {{ $variant->is_active ? __('Active') : __('Inactive') }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-end">
                                    <a href="{{ route('admin.products.variants.edit', [$product, $variant]) }}" class="rounded-lg px-2 py-1 text-xs font-semibold text-amber-700 hover:bg-amber-50">{{ __('Edit') }}</a>
                                    <form action="{{ route('admin.products.variants.destroy', [$product, $variant]) }}" method="post" class="inline" onsubmit="return confirm(@json(__('Delete this variant?')));">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="6" class="px-4 py-8 text-center text-slate-500">{{ __('No variants found.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
