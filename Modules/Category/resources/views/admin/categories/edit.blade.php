                ...
                <div class="lg:col-span-2 space-y-10">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label
                                        class="block mb-1.5 text-sm font-semibold text-slate-700">{{ __('Parent Category') }}</label>
                                    <select name="parent_id"
                                        class="w-full rounded-xl border border-slate-300 focus:border-slate-500 focus:ring focus:ring-slate-200 px-4 py-3 transition-all">
                                        <option value="">{{ __('None (Top level)') }}</option>
                                        @foreach ($parents as $parentId => $parentLabel)
                                            <option value="{{ $parentId }}" @selected((string) old('parent_id', $category->parent_id) === (string) $parentId)>
                                                {{ $parentLabel }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label
                                        class="block mb-1.5 text-sm font-semibold text-slate-700">{{ __('Menu order') }}</label>
                                    <input type="number" name="menu_order"
                                        value="{{ old('menu_order', $category->menu_order) }}"
                                        class="w-full rounded-xl border border-slate-300 focus:border-slate-500 focus:ring focus:ring-slate-200 px-4 py-3 transition-all">
                                    <p class="mt-1 text-xs text-slate-500">
                                        {{ __('Lower numbers appear first in the storefront sidebar.') }}</p>
                                </div>
                                <div class="flex items-end pb-1">
                                    <label class="inline-flex items-center gap-3 cursor-pointer">
                                        <input type="hidden" name="show_in_sidebar" value="0">
                                        <input type="checkbox" name="show_in_sidebar" value="1"
                                            @checked(old('show_in_sidebar', $category->show_in_sidebar))
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="4"
                                    class="w-full rounded-xl border border-slate-300 focus:border-slate-500 focus:ring focus:ring-slate-200 px-4 py-3 transition-all">{{ old('description', $category->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-6 space-y-6">
                            <div>
                                <label
                                    class="block mb-1.5 text-sm font-semibold text-slate-700">{{ __('Meta Title') }}</label>
                            ...
                            <div>
                                <label
                                    class="block mb-1.5 text-sm font-semibold text-slate-700">{{ __('Meta Description') }}</label>
                                <textarea name="meta_description" rows="3"
                                    class="w-full rounded-xl border border-slate-300 focus:border-slate-500 focus:ring focus:ring-slate-200 px-4 py-3 transition-all">{{ old('meta_description', $category->meta_description) }}</textarea>
                            </div>
                        </div>
                    </div>
